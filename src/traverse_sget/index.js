...
function transformPath(objectString, argList) {
    if (argList.length) {
        ...
        const condExpressionString = `${objectString} === undefined ? void 0 : ${transformPath(objectString, argList.slice(1))}`
...
function transformTargetObject(node, path) {
    return function (result) {
        if (t.isMemberExpression(node)) {
            ...
            return template(`
            ${newPath.join('.')} === undefined ? void 0 : ${generator.default(transformTargetObject(node.property, newPath)(result)).code}
        `)()
        } else {
            return template(`
